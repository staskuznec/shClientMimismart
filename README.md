...
авторизация по ключу AES и отправка значений — текстовых статусов, показаний
датчиков, однобайтовых состояний и произвольных полезных нагрузок.
...
Форму значения диктует элемент, а не удобство вызывающего кода:

| Конструктор | Что уезжает | Для чего |
|---|---|---|
| `Sensor(id, sub, v)` | 2 байта, fixed-point 8.8 | датчики: температура, влажность, ток |
| `Text(id, sub, s)` | строка как есть | текстовые статусы |
| `Byte(id, sub, v)` | ровно 1 байт | лампа, скрипт, клапан, шторы, ворота, датчики двери и протечки |
| `Raw(id, sub, p)` | произвольные байты | всё остальное |

`Sensor` и `Byte` перепутать особенно легко, а последствия неочевидны:

```go
shclient.Sensor(773, 1, 1) // → 00 01 — младший байт нулевой
shclient.Byte(773, 1, 1)   // → 01
```

Однобайтовый элемент читает из пакета датчика первый байт, то есть ноль, и
остаётся выключенным. Выключение при этом «работает», потому что ноль даёт нули
в обоих байтах — выглядит как статус, пропадающий через раз.

У `Byte` ошибки нет: один байт помещается в пакет всегда.
...
элементами, их адресами, именами и типами:

```xml
<item addr="773:1" cfgid="17" name="Прихожая" type="lamp"/>
<item addr="542:16" cfgid="40" name="д.т. Прихожая" type="temperature-sensor"/>
```

По умолчанию клиент его отбрасывает. Чтобы получить — задайте приёмник; байты
пишутся потоком, поэтому это может быть файл, и весь документ в памяти держать
не обязательно:

```go
f, err := os.Create("logic.xml")
if err != nil {
	return err
}
defer f.Close()

c, err := shclient.New(addr, key, shclient.WithLogicSink(f))
```
